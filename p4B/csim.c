...
void access_data(mem_addr_t addr) {
    addr = addr>>b;
    unsigned long long setIndex  = addr& (0x7fffffffffffffff>>(63-s));
    addr = addr>>s;
    unsigned long long addr_t = addr;
	int emptyLine = -1;
	int hit = 0;
	int maxLRU = 0;
	int minPositon = -1;
	 
	//find set mapped to the address
	cache_line_t* currentSet = cache[setIndex];
	for (int j=0; j<E; j++) {
		cache_line_t currentLine = currentSet[j];
        ...
        if (currentLine.valid == 1 && currentLine.tag == addr_t) {
			hit = 1;
            hit_cnt++;
    ...
    if (emptyLine != -1 && hit == 0) {
		cache_line_t addLine = {1,addr_t,0};
    ...
    if (emptyLine == -1 && hit == 0) {
		...
		cache_line_t replaceLine = {1,addr_t,0};
        cache[setIndex][minPositon] = replaceLine;
		miss_cnt++;
        evict_cnt++;
    }
}
